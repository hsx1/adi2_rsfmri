...
Higher BMI and obesity have been associated with differences in DMN FC in several resting state and task-based fMRI studies [@Beyer_2017; @Kullmann_2012; @Chao_2018; @Wijngaarden_2015, @Borowitz_2020; @Ding_2020; @Doucet_2017; @Sadler_2018]. 
...
tab_sample <- mk_SampleTable(final)
...
carpets=c("/data/pt_02161/Analysis/Project2_resting_state/seed-based/Second_level /code_and_manuscript/report/carpetplotadi006fu2.jpeg","/data/pt_02161/Analysis/Project2_resting_state/seed-based/Second_level /code_and_manuscript/report/carpetplotadi16fu.jpeg")
...
```{r calculate summary of QA, eval=FALSE, echo=FALSE}
...
final=read.csv(file = "./report/all_QA_measures.csv")

FigureList <- figDvarsmFD(final)
(FigureList$fig1 | FigureList$fig2) / FigureList$fig3
```
...
#networks=c("./report/DMN_thresholded_for_clusterFWE005.png","./report/Rew_thresholded_for_clusterFWE005.png")
networks=c("/data/pt_02161/Analysis/Project2_resting_state/seed-based/Second_level /code_and_manuscript/report/DMN_thresholded_for_clusterFWE005.png", "/data/pt_02161/Analysis/Project2_resting_state/seed-based/Second_level /code_and_manuscript/report/Rew_thresholded_for_clusterFWE005.png", "/data/pt_02161/Analysis/Project2_resting_state/seed-based/Second_level /code_and_manuscript/report/tsnr.jpeg")
...
```{r prepare tsnr ROI plot, eval=FALSE}
...
p<-ggplot(aes(as.factor(roi), mean),data=final_tsnr) +
  ...
  scale_fill_manual(values=wes_palette("Darjeeling1",5,type="discrete")[3:4]) +
  theme(axis.text=element_text(size=10),
  axis.title=element_text(size=12),
  strip.text = element_text(size=12),
  legend.position="")
ggsave("/data/pt_02161/Analysis/Project2_resting_state/seed-based/Second_level /code_and_manuscript/report/tsnr.jpeg", p)
```
...
There was no interaction of group and time point regarding the resting state connectivity for neither the PCC nor the NAcc seeds against our initial hypothesis. There also was no significant main effect for any of the covariates of interest (time, group) in clusterwise inference with FWE correction.

####  Model 1b (with adjustment for age and sex, meanFD)
Similarly, there were no significant clusters when including log mean FD. Results did not differ between AROMA+CC and AROMA+CC+GSR preprocessing pipelines.
...
To disentangle effects of change in BMI and logmFD in this dataset, we also split mFD into average mFD and change in mFD. Age and sex remained as nuisance covariates.
<!-- ### DMN -->
...
Test statistics of these clusters differed only slightly from the effect found for average BMI when using mFD as nuisance covariate.
<!-- gsr -->As before, the effect did not survive when using GSR-corrected data.

<!-- ### Reward -->
<!-- cc -->
For the reward network, we found a significant negative association of change in BMI with FC between NAcc and a cluster in the posterior-medial frontal region, i.e. supplementary motor area (see Table \@ref(tab:tableBMIFDmodel)). Voxel activation at local maximum within this cluster was significant at peak level after FWE-correction. 
Moreover, FC between the NAcc and one cluster in a more posterior region, i.e. motor cortex, was significantly positively related to average mFD.
When running the model with GSR-preprocessed data, the associations between change in BMI and FC did not survive statistical thresholds. The relationship with average mFD was confirmed by analyses after GSR but cluster size shrank.
...
```
The association also remained significant when we split logmFD into average and change in logmFD (p = `r round(p_mean_BMI,4)`) and there was no significant association of mean or change in logmFD with DMN FC.
...
vif_res=vif(test_vif)
``` 
As expected from the whole-brain findings, there was no association of mean BMI or within-subject BMI change and reward network connectivity in models adjusting for age, sex and logmFD.
...
```{r tableBMImodel, echo=TRUE}
source("report/tables.R")
TableList <- mk_FCTables()
tab_BMImodel <- TableList[[1]]
tab_BMImodel
```

```{r tableBMIFDmodel, eval=FALSE}
tab_BMIFDmodel <- mk_FCTables()[[3]]
tab_BMIFDmodel
```

```{r tableFDmodel, eval=FALSE}
tab_FDmodel <- mk_FCTables()[[2]]
tab_FDmodel
```
