...
# [x] re-do analysis with new sample
...
figBMIdescr <- mk_figBMIdescr(final)
...
Figure \@ref{fig:figNetworks} shows the DMN and reward network which we used for extracting the aggregated FC valued. Both networks include previously described brain regions (i.e. medial prefrontal cortex, parietal lobules and posterior hippocampus for the DMN, and anterior cingulare, right amygdala and ventromedial-prefrontal cortex for the reward network). Yet, the reward network is less pronounced (overall lower T-values) and symmetric (no significant connectivity with left amygdala), which might be due to SNR differences in the seed brain regions (see Figure \@{ref:fig:figtSNR}.
>>>>>>> include glass brain of networks and LLMMs into draft

```{r figNetworks, echo=FALSE, out.width="49%", out.height="30%", fig.cap="Resting state networks based on AROMA+CC+GSR input data, one-sample t-tests adjusting for age and sex over all timepoints and participants with bootstrapped clusterwise inference (p_FWE<0.05)",fig.subcap=c('DMN (seeded in bilateral precuneus)','Reward network (seeded in bilateral Nucleus Accumbens'),fig.show='hold',fig.align='center'}
networks=c("/data/pt_02161/Results/Project2_resting_state/connectivity/Analysis/aggFC/DMN/DMN_thresholded_for_clusterFWE0.05.png","/data/pt_02161/Results/Project2_resting_state/connectivity/Analysis/aggFC/Rew/Rew_thresholded_for_clusterFWE0.05.png")
knitr::include_graphics(networks)
```
```{r prepare tsnr ROI plot, echo=FALSE}
...
```{r figtSNR,fig.cap="Lower tSNR in the region of interest NAcc versus precuneus used for seed-based connectivity (shown are all 101 timepoints)", fig.width=6, fig.height=4,  echo=FALSE}
p
```
...
<!-- Neurovault -->
Unthresholded maps for the t-tests of the DMN and reward network (which were used for data aggregation) as well as contrasts of group, time and group-by-time interaction will be published on NeuroVault.
...
final_FC.meanFD=tapply(X=final_FC$logmFD,
                        INDEX=final_FC$subj.Nr, FUN=mean, na.rm=TRUE)
final_FC$mean.logmFD=
  final_FC.meanFD[as.numeric(final_FC$subj.Nr)]
...
fig1=fig_DMNconn +
  ...
  xlab("Time [months]") + ylab ("mean DMN connectivity") +
  theme_bw() +  theme(
...
fig2=fig_Rewconn +
  ...
  xlab("Time [months]") + ylab ("mean reward network connectivity") +
  theme_bw() +  theme(
    axis.text = element_text(size = 10),
    axis.title = element_text(size = 12),
    strip.text = element_text(size = 12),
...
tab_FDmodel <- TableList[[2]]
tab_FDmodel
```

```{r tableBMIFDmodel, echo=FALSE}
tab_BMIFDmodel <- TableList[[3]]
tab_BMIFDmodel
```
