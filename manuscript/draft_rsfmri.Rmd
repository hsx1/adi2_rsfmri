...
Most resting-state fMRI studies in obesity have reported an association of higher BMI and higher head motion during MRI [@Beyer_2017;@Hodgson_2016]. Head motion is a major confounder in resting-state fMRI as it increases short-range and disruptes long-range connectivity FC. In this sample, we previously reported a group-by-time interaction on head motion [@Beyer_2020]. Here, we thus aimed to rigorously control for motion-related variance. 
...
As preregistered, we performed the analysis for baseline and followup time points only, in both denoising pipelines and adjusting for no confounders (CA 1), age, sex and average mFD of both timepoints (CA2) and age,sex, average mFD and baseline BMI (CA3). 
 In this model, time was represented as factor because we took into account possible non-linear time courses in the increase and decrease of FC over the course of one year, which may occur depending on the phase of weight management (Olivo et al. 2017). The resulting factorial design contained one regressor for each time point per group (see supplements for depiction of design matrix). 
...
For each brain network, we examined the models for the two denoising pipelines (AROMA+CC / AROMA+CC+GSR)as preregistered. For all analyses, we used an explicit brain mask, which was a whole brain derived from the MNI ICBM “152 nonlinear 6th generation” atlas (re-sampled to 3 x 3 x 3 mm$^3$ and thresholded at 0.5 GM probability).<!--  This resulted in a number of tests (voxels of interest) from $52378$. MNI ICBM 2009a Nonlinear Symmetric grey matter mask ([http://www.bic.mni.mcgill.ca/ServicesAtlases/ICBM152NLin2009](http://www.bic.mni.mcgill.ca/ServicesAtlases/ICBM152NLin2009)) were re-sampled to 3 x 3 x 3 mm$^3$ and binarized at threshold $0.5$. -->
