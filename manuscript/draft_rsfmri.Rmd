...
fig_Rewconn
```
There was no significant time by group interaction for aggregated DMN and reward network FC (see Figure \@ref(fig:figaggFC) and supplements for a detailed summary of the models)
...
There was no significant time by group interaction effect for aggregated DMN and reward network FC (see Figure \@ref(fig:figaggFC)) and supplements for a detailed summary of the models).
...
```
Moreover, FC between the NAcc and one cluster in a more posterior region, i.e. motor cortex, was significantly positively related to average logmFD. This result remained similar when using GSR corrected data.
...
```
Corresponding to the whole brain results, there was a negative association of average BMI and aggregated DMN connectivity ($p$ = `r round(p_meanBMI,3)`), regardless of whether we adjusted for logmFD.
...
As expected from the whole-brain findings, there was no association of average BMI or within-subject BMI change and reward network connectivity in models adjusting for age, sex and logmFD.
...
<!-- #### Reward -->
As shown in Table \@ref(tab:tableFDmodel), there was a significant association between logmFD and co-activation of NAcc with one cluster located in proximity to the central sulcus and motor areas . Cluster co-activation appeared irrespective of the preprocessing procedure, differing mainly in cluster size.
<!-- ### DMN -->
...
<!-- Average BMI and change in BMI remain covariates of interest. While initially used as nuisance covariate, here, logmFD is split into average logmFD and change in logmFD and used as. Age and sex remain nuisance covariates. -->
...
<!-- Voxel activation at local maximum within this cluster was significant at peak level after FWE-correction. Furthermore, the functional connectivity between the NAcc and one cluster was significantly related to average logmFD. -->
...
<!-- On the other hand, the relationship with average logmFD was confirmed by analyses after GSR but cluster size shrank. -->
