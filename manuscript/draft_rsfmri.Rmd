...
<!-- STUDY DESIGN --> The ADIPOSITAS-Study investigated the effects of bariatric surgery on brain structure and function in a prospective design at the Charité Berlin. For more details see Prehn et al. -@Prehn_2020. We used all data acquired until April 2019.  
...
<!-- SURGICAL PROCEDURE --> As intervention, $?$ participants received surgery that combines restrictive and malabsorptive mechanisms (RYGB) and $?$ surgery purely restricting energy intake (VSG: n = $?$, GB: n = $?$).

<!-- STUDY PROCEDURE --> Subjects arrived in the morning (between 07:00 and 12:00) after an overnight fast. They underwent medical assessments including an interview, blood draw and anthropometric measurements before having a one our break for <!-- standadized??  -->breakfast. MRI scanning was done after further psychological tests (for details see Prehn et al. -@Prehn_2020).
...
We did not perform the analysis as preregistered (i.e. without covariates and additional sensitivity analyses), but adjusted for baseline age and sex (Model 1a), and baseline age, sex and the logarithm of mean FD (called logmFD in the following) (Model 1b).
...
For each brain network, we examined the models for the two preprocessing pipelines (AROMA+CC / AROMA+CC+GSR), resulting in $8$ preregistered analyses, and $16$ <!-- bmi change (2 models), bmi + mfd change, fd change=4 * 2* 2 --> exploratory whole brain analysis.
...
Further, we performed an exploratory analysis with average BMI and variability in BMI for the aggregated FC, in line with our whole brain analysis, and also included average logmFD and change in logmFD.
...
Yet, the reward network was less pronounced (overall lower T-values) and symmetric (no significant connectivity with left amygdala), which might be due to lower SNR in the seeded brain region (see supplements).
...
####  Model 1b (adjusted for age, sex and logmFD)
...
##### Model 2a and 2b (average and change in BMI, adjusted for logmFD)
...
##### Model 2c (average and change in BMI and split logmFD)
...
### Effect of logmFD (exploratory analysis)
