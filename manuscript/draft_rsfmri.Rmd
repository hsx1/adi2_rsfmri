...
In sum, while there is some evidence hinting to a role for DMN and RN connectivity in altered regulation of food intake after bariatric surgery, the existing evidence is inconclusive. Most studies have investigated small cohorts of patients, without adequate obese control groups, and did not rigorously separate confirmatory from exploratory analyses [@George_2016].
Most resting-state rsfMRI studies in obesity have reported an association of higher BMI and higher head motion during MRI [@Beyer_2017;@Hodgson_2016]. Head motion is a major confounder in resting-state rsfMRI as it increases short-range and disruptes long-range connectivity FC. In this sample, we previously reported a group-by-time interaction on head motion [@Beyer_2020]. Here, we thus aimed to rigorously control for motion-related variance. 
In this pre-registered study, our hypotheses were:
1. Whole-brain FC of the RN and DMN changes differently from baseline to followup in the bariatric surgery compared to waiting-list control group
2. Within-network FC of the RN and DMN changes differently from baseline to followup in the bariatric surgery compared to waiting-list control group
...
<!-- STUDY DESIGN --> The ADIPOSITAS-Study investigated the effects of bariatric surgery on brain structure and function in a prospective design at the Charité University Medicine Berlin, Germany. For more details see Prehn et al. (2020). We used all data acquired until April 2019.
...
#### Resting-state fMRI quality control
Resting-state rsfMRI quality control was performed according to the protocol by @Ciric_2018. 
Head motion is a major confounder in rsfMRI. We calculated mean framewise displacement (mFD) from the outputs of `MCFLIRT` according to @Power_2012.
...
Further, DVARS did not qualitatively differ between groups and time points, and the correlation of DVARS and mFD remained similar over time points (see Figure \@ref(fig:figDvarsmFD).
Following the protocol by [@Ciric_2018], we evaluated mFD-FC relationships by computing a functional connectome based on [@Power_2012]. In more detail, we used the MNI coordinates of 246 cortical and subcortical spherical seed regions with 5mm radius in `NiftiSpheresMasker`, and calculated the Spearman rank correlation between these time series. Further, we evaluated the distance-dependence of mFD-FC correlations by correlating the euclidean distance between nodes with the mFD-QC correlation and report mean, median and standard deviations of mFD, DVARS and number of outliers over all participants.
```{r mFDFC, echo=FALSE}
...
tab_Qcfc
```
Table \@ref(tab:tableQcfc) shows that in the minimally preprocessed data, mFD and FC between nodes were significantly positively correlated (mean Spearman's $r$ = `r round(res[1,1],2)`). This relationship was negatively associated with distance between nodes (mean Spearman's $r$ = `r round(res[1,5],2)`).  
Denoising pipelines (AROMA, AROMA+CC, AROMA+CC+GSR) gradually reduced the correlation of mFD and connectivity, and reduced the distance dependency. Contrary to previous reports, GSR did not exercabate mFD-FC distance dependency [@Power_2015]. Overall, AROMA+CC+GSR performed best, with almost no mFD-FC correlation (mean Spearman's $r$ = `r round(res[4,1],2)`), and minimal positive distance dependence (mean Spearman's $r$ = `r round(res[4,5],2)`)
...
We tested the preregistered hypothesis of a  time by group interaction:  $FC=group + time + group * time$. 
As preregistered, we performed the analysis for baseline and followup time points only, in both denoising pipelines and adjusting for no confounders (CA 1), age, sex and average mFD of both timepoints (CA2) and age,sex, average mFD and baseline BMI (CA3). 
...
Then, we extended our whole-brain analysis of the group-by-time point interaction to the data from all three timepoints. This analysis had not been preregistered. Here, we used individual mFD values (not averaged) as covariate to capture variance in mFD change over time points.We investigated two models (EA1.1: group x time, age, sex and EA1.2: group x time, age, sex and mFD). 

##### Average and change BMI
We further  explored the relationship of average and change in BMI in a separate group of models. As proposed by @Guillaume_2014, we calculated the between- and within subject centered values of BMI. This model, containing average BMI and BMI variability, allowed us to disentangle the differential effects of overall BMI and change in BMI on FC. We first estimated their effects in a model adjusting for baseline age and sex (Model EA2.1) and then additionally controlling for log mean mFD (Model EA 2.2). As we previously reported correlated change in BMI and mFD in this sample [@Beyer_2019], we explored a refined model including average BMI and logmFD and change in both measures, along with baseline age and sex (Model EA 2.3c). Here, we aimed to see whether any effect of change in BMI would be detectable when adjusting for the change in head motion. 
For a better understanding of the unique contribution of average and longitudinal change FD measures, we separately employed a third post-hoc, exploratory model (EA 2.4), solely including the head motion measures as covariates of interest: $FC = between-subject FD + within-subject FD$ age and sex as nuisance covariates.
...
For each brain network, we examined the models for the two denoising pipelines (AROMA+CC / AROMA+CC+GSR)as preregistered. For all analyses, we used an explicit brain mask, which was a whole brain derived from the MNI ICBM “152 nonlinear 6th generation” atlas (re-sampled to 3 x 3 x 3 mm$^3$ and thresholded at 0.5 GM probability).
...
First, we performed the preregistered analysis in participants with baseline and followup data, using two denoising pipelines and three covariate adjustements.
...
###  Model CA2 (adjusted for age, sex and average of mFD)
...
###  Model CA3 (adjusted for age, sex, average of mFD and baseline BMI)
There was no significant interacion when adding baseline BMI.
...
Similarly to the analysis with two time points, there was no significant interaction or main effect when analyzing models EA 1.1 and EA 1.2 in the full data set for neither reward network nor DMN.
...
###### Average and change in BMI, adjusted for split mFD (EA 2.3)
<!-- cc -->For the reward network, we found a significant negative association of change in BMI with FC between NAcc and a cluster in the posterior-medial frontal region. 
...
```{r "BMIFDagesexNACCcgnBMIdeact", fig.cap="Significant cluster with a negative Effect of change BMI on NAcc to posterior-medial frontal region, adjusted for age, sex, mean BMI and logmFD", fig.subcap = c("in AROMA+CC", "in AROMA+CC+GSR data"), out.width = "50%", fig.hold=TRUE, eval=TRUE}
...
```
As expected from the whole-brain findings, there was no association of average BMI or within-subject BMI change and within reward network connectivity in models 2a, 2b or 2c adjusting for age, sex and logmFD.
#### DMN
##### Models EA 2.1, 2.2. and 2.3
...
```
Corresponding to the whole brain results, there was a negative association of average BMI and aggregated DMN connectivity ($p$ = `r round(p_meanBMI,3)`), regardless of whether we adjusted for logmFD (model EA 2.1 and EA 2.2). 
...
All code is available in the github repository [https://github.com/hsx1/adi2_rsrsfMRI](https://github.com/hsx1/adi2_rsrsfMRI). The code includes script for data preparation and analysis of different models.
