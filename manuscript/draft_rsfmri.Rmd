...
Five data points were excluded due to failure of correct preprocessing attributed to T1-artifacts.
For significant group-time
...
<!-- DESIGN MATRIX, vielleicht beide auch als Abbildung kurz zeigen? --> For the first model, time was introduced as factor because did not assume strict linearity for the increase in connectivity and therefore refrained from modelling time as continous variable. The resulting factorial design <!-- check: flexible factorial? --> contained one regressor for each time point per group.
```{r "Plot design matrices", fig.align='center', out.width="90%"}
...
dmf <- data.frame(ID,condition,KG_bl,KG_fu,KG_fu2,IG_bl,IG_fu,IG_fu2)
colnames(dmf) <- c("ID","condition","KG_bl","KG_fu","KG_fu2","IG_bl","IG_fu","IG_fu2")
dmf$measurement <- c(1:nrow(dmf))
...
dmc <- data.frame(ID,condition,group_IG,group_KG,tp_IG,tp_KG)
colnames(dmc) <- c("ID","condition","group_IG", "group_KG","tp_IG","tp_KG")
dmc$measurement <- c(1:nrow(dmc))

# plot dmf
input_dmf <- dmf %>%
  pivot_longer(
    cols= c("KG_bl","KG_fu","KG_fu2","IG_bl","IG_fu","IG_fu2"),
    names_to = "regressor")
input_dmc <- dmc %>%
  pivot_longer(
    cols= c("group_IG", "group_KG","tp_IG","tp_KG"),
    names_to = "regressor")
...
Unlike the description in the preregistration, we used a cluster forming threshold of $p < 0.001$ for a more multiple comparison adjustment and $1000$ bootstraps due to required computation time. Significant clusters are defined as family-wise error (FWE) corrected $p < 0.05$.
