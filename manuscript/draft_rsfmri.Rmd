...
Statistical analysis were performed in MATLAB version 9.7.0.1190202 (R2019b, @MATLAB2018) using the development version of the SwE toolbox [@Guillaume_2014] ([https://github.com/NISOx-BDI/SwE-toolbox](https://github.com/NISOx-BDI/SwE-toolbox) as of commit #384d2d7944e0c1919a3d22a0282a513916c25f59 was used because of bugs in most recent release) as implemented in the Statistical Parametric Mapping software (SPM12.7770, @spm12)<!-- spm_check_installation('rev') -->, and R version 3.6.1 [@team2013r].
...
We also inspected DVARS, a measurement of whole-brain variation in signal which is often reflective of noise in the data. We saw that DVARS did not qualitatively differ between groups and time points, and that the correlation of DVARS and mFD remained similar over time points.
...
Similarly, there were only non-findings when reducing the number of nuisance covariates by leaving out mean FD. Results did not differ between analyses of connetivity maps from different preprocessing pipelines.
...
deact_file <- "results_deact.csv"
act_file <- "results_act.csv"
```
...
df1 <- read.csv(file.path(result_dir,BMImodel_dirs[1],deact_file))
colnr <- ncol(df1)
df1$seed <- NULL
...
df2 <- read.csv(file.path(result_dir,BMImodel_dirs[2],deact_file))
df2$seed <- NULL
df2 <- rbind(rep(NA, colnr),df2)
df2[1:2,"seed"] <- c("\\textit{average BMI (decrease)}","PCC (cc)")
df2$covariates <- NA
df2$covariates[2] <- "age, sex"
...
df3 <- read.csv(file.path(result_dir,FDBMImodel_dirs[1],deact_file))
df3$seed <- NA
 df3 <- rbind(rep(NA, colnr),df3)
df3[1:2,"seed"] <- c("\\textit{change in BMI (decrease)}","NAcc (cc)")
df3$covariates <- NA
df3$covariates[2] <- "age, sex"
# BMIFDagesex_NACCcc_avgFD_act
 df4 <- read.csv(file.path(result_dir,FDBMImodel_dirs[2],act_file))
df4$seed <- NA
df4 <- rbind(rep(NA, colnr),df4)
df4[1:2,"seed"] <- c("\\textit{average log mean FD (increase)}","NAcc (cc)")
df4$covariates <- "age, sex"
# BMIFDagesex_NACCgsr_avgFD_act 
df5 <- read.csv(file.path(result_dir,FDBMImodel_dirs[3],act_file))
df5$seed <- NA
df5 <- rbind(rep(NA, colnr),df5)
df5[1:2,"seed"] <- c("\\textit{average log mean FD (increase)}","NAcc (gsr)")
df5$covariates <- NA
df5$covariates[2] <- "age, sex"
# BMIFDagesex_PCCcc_avgBMI-deact 
df6 <- read.csv(file.path(result_dir,FDBMImodel_dirs[4],deact_file))
df6$seed <- NA
df6 <- rbind(rep(NA, colnr),df6)
df6[1:2,"seed"] <- c("\\textit{average BMI (decrease)}","PCC (cc)")
df6$covariates <- NA
df6$covariates[2] <- "age, sex"

# BMIFDagesex_NACCcc_avgFD_act 
df7 <- read.csv(file.path(result_dir,FDmodel_dirs[1],act_file))
df7$seed <- NA
df7 <- rbind(rep(NA, colnr),df7)
df7[1:2,"seed"] <- c("\\textit{average log mean FD (increase)}","NAcc (cc)")
df7$covariates <- NA
df7$covariates[2] <- "age,sex"
# BMIFDagesex_NACCgsr_avgFD_act <
df8 <- read.csv(file.path(result_dir,FDmodel_dirs[2],act_file))
df8$seed <- NA
df8 <- rbind(rep(NA, colnr),df8)
df8[1:2,"seed"] <- c("\\textit{average log mean FD (increase)}", "NAcc (gsr)")
df8$covariates <- NA
df8$covariates[2] <- "age, sex"
...
```{r include = FALSE}
model_list <- list(BMImodel,FDmodel,BMIFDmodel)

for (i in 1:length(model_list)){
  model_list[[i]]$anat <- NA
  model_list[[i]]$hemisphere <- NA
  # select columns
  model_list[[i]] <- model_list[[i]][,c("seed","covariates","anat","hemisphere","cluster_p.FWE.corr.","cluster_equivk","cluster_equivkZ","peak_Z","X_x","y","z..mm.")]
  # name columns
  colnames(model_list[[i]]) <- c("Seed","Covariates","Anatomical region","L/R","clusterwise FWE-corr. P","cluster size","Z score of cluster size","Z score in local maximum","X","Y","Z") 
}

for (i in 1:length(model_list)){
  # only keep cluster statistics and peak local maximum
  model_list[[i]] <- model_list[[i]][!is.na(model_list[[i]]$Seed) | !is.na(model_list[[i]]$`clusterwise FWE-corr. P`),]
}

BMImodel <- model_list[[1]]
FDmodel <- model_list[[2]]
BMIFDmodel <- model_list[[3]]
...
```{r tabModel}
# hide NA in tables
fn1 <- "R, right hemisphere;L, left hemisphere; MNI (Montreal Neurological Institute) coordinates of primary peak location; x = sagittal, y = coronal,"
fn2 <- "z = axial. To identify significant clusters, we applied a cluster size threshold with p < 0.001 determined by Wild Bootstrap of 1000 samples."
...
for (i in 1:length(model_list)){
  table_list[[i]] <-
    ...
    knitr::kable(
      ...
      col.names = colnames(model_list[[i]]),
      row.names = FALSE,
      format = "latex",
      booktabs = T,
    ...
    column_spec(c(3, 5, 7, 8), width = "2.4cm") %>%
    add_footnote(c(fn1, fn2), notation = "none") %>%
    kableExtra::landscape()
}
tab_BMImodel <- table_list[[1]]
tab_FDmodel <- table_list[[2]]
tab_BMIFDmodel <- table_list[[3]]
```

```{r}
group <- c(1, NA, NA, 2, NA, NA)
quest <- c("How is your mood today?", "good or very good", "bad or very bad","What colour is your hair?", "brown", "other")
percent <- c(NA, 80, 20, NA, 50, 50)
df <- tibble(group, quest, percent)
df <- data.frame(quest, percent) 

tab_BMImodel %>%
  group_rows("average BMI (decrease)", 1, 3) %>%
  group_rows("average BMI (decrease)", 4, 7)
...
```{r tabBMIFDmodel, echo = FALSE}
tab_BMIFDmodel
```

<!-- Brain mask: average and individual BMI [OLD] -->
<!-- BMItotal --> Next, we tested for significant effects of the average BMI and the individual change in BMI longitudinally across the three time points. We did not find a significant co-activation for neither the PCC nor the NAcc associated with average BMI nor change in BMI.
...
Surprisingly, we did not find a significant effect neither for the total sample, nor for the intervention group only.
...
<!-- model specific --> With the model on time-by-group--interaction, we primarily investigated whether changes in functional connectivity over time vary depending whether participants have received surgery or not, while controlling for individual variance in resting-state connectivity over time and at baseline. Beyond that, also main effects of the intervention and of the time were examined. <!-- correct interpretation of marginal models? -->
